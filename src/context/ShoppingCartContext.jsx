...
export function ShoppingCartProvider({ children }) {
    ...
    function getItem(id) {
        return products.find(item => {
            ...
            return item._id === id || item.id === id || item._id?.toString() === id?.toString();
        });
    }
    ...
    function increaseCartQuantity(id) {
        setCartItems(currItems => {
            if (currItems.find(item => item.productId === id || item.id === id) == null) {
    ...
    function decreaseCartQuantity(id) {
        setCartItems(currItems => {
            if (currItems.find(item => item.productId === id || item.id === id)?.quantity === 1) {
                return currItems.filter(item => item.productId !== id && item.id !== id);
            } else {
                return currItems.map(item => {
                    if (item.productId === id || item.id === id) {
                        return { ...item, quantity: item.quantity - 1 };
